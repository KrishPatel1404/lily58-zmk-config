...
- **2026-07-23** — 🐛 Two encoder-scroll bugs found on hardware. (1) **Scroll did nothing**: `&msc` is `zmk,behavior-input-two-axis` (velocity/time based, NOT a tap keycode) with stock `time-to-max-speed-ms=300`; a sensor tap of `tap-ms=30` accumulates <1 wheel unit → zero scroll. Fix: top-level `&msc` label override `time-to-max-speed-ms=0` + `delay-ms=0` (instant full speed) and raise `scroll_enc` tap-ms 30→70. Verified vs JinRecords/zmk-config-Jin working config. NOTE: `&msc {...}` override MUST be top level (outside `/ {}`) — nesting it in root is invalid DTS. (2) **Right half stopped connecting**: `CONFIG_ZMK_POINTING=y` enables `ZMK_INPUT_SPLIT` (default y), which changes the split BLE service — so a pointing-enabled central can't talk to a pre-pointing peripheral. Any feature that flips a split-protocol Kconfig ⇒ **flash BOTH halves**, not just central. My earlier "encoder = flash left only" advice was wrong for this change.
