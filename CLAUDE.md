...
- `config/lily58.keymap` — 3 layers (Base / Lower / Raise). No reserved ZMK Studio runtime-layer slots — removed 2026-07-23 per Krish (adding layers live in Studio now needs a reflash; recoverable from git if wanted). **Homerow mods** on A/S/D/F + J/K/L/; via custom `hml`/`hmr` hold-taps (Mac order: Ctrl-Shift-Opt-Cmd mirrored; balanced flavor, tapping-term 280 ms, require-prior-idle 150 ms; opposite-hand triggers PLUS same-hand homerow-mod positions so same-hand mods stack, e.g. Cmd+Shift — added 2026-07-23). No combos/macros yet. **Rotary encoder** (EC12, no push button) on the LEFT half — now a **`gpio-qdec` INPUT device** (NOT a keymap sensor) doing **real mouse-wheel scroll on all layers** (`left_qdec` + `zmk,input-listener` in the keymap; `CONFIG_ZMK_POINTING=y` + `CONFIG_INPUT_GPIO_QDEC=y`; stock `&left_encoder`/`&sensors` disabled). Trade-off: an input-device encoder can't emit consumer keycodes on v0.3.0, so knob brightness/volume were dropped (can't have both true-wheel AND keycodes on one knob — see [[encoder-mouse-scroll-macos]]). Original EC11 sensor was hardware-verified 2026-07-23; gpio-qdec rewrite 2026-07-23, NOT flash-tested. 
