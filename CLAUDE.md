...
- **Rotary encoder — LEFT half only** (branch `rotary`, not yet on main / not flash-tested). Stock ZMK v0.3 lily58 shield already defines `left_encoder` (`alps,ec11`, `pro_micro 21`=A / `20`=B, steps 80) + `sensors` node (`triggers-per-rotation=20`, lists only left), and `lily58_left.overlay` sets it `okay` — so enabling it needs NO overlay/node redefinition: just `CONFIG_EC11=y` + `CONFIG_EC11_TRIGGER_GLOBAL_THREAD=y` in `.conf` and one `sensor-bindings` line per keymap layer. Encoder works ONLY on the left (its pads are the left nice!nano's `pro_micro 20/21`); right half never has one. Wrong direction? swap the two keycodes. Hand-solder guide + wiring in memory (`rotary-encoder-future-add`) and the wiring artifact.
